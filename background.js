...
function launch(launchData) {
	...
	// If launched via a url_handler, we need to grab data passed through the URL.
	...
	if (launchData.source === 'url_handler' && launchData.url && launchData.id) {
		var regex = new RegExp('^http[s]?:\/\/' + launchData.id, 'i');
		relativeUrl = launchData.url.replace(regex, '');
	}
